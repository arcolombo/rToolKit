...
###add activation direction
  activation.direction<-matrix(data=0,nrow=nrow(moduleTraitCor2),ncol=length(names(xN)))
  rownames(activation.direction)<-rownames(moduleTraitCor2)
