...
##read MSigDB selection from input
  ...
  ##collapse duplicated gene names with unique EnsIDs
    if(any(duplicated(rownames(cnts_mt)))){
     message("found duplicated hgnc names with differen EnsG ids")
    }
     cnts_pL2<-cnts_mt[which(rownames(cnts_mt)!=""),]
  ##FIX ME:: filter out "" gene names,  and sum up the duplicate gene names as an aggregate
    dupes<-rownames(cnts_pL2[which(duplicated(cnts_pL2)==TRUE),])
    cnts_pL3<-cnts_pL2[which(duplicated(cnts_pL2)==FALSE),]
  ...
  par(mar = c(10, 6, 3, 3));
  plot(qs.results)
   title(paste0(module,".",tx.biotype))
