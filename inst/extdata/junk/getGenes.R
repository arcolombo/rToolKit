...
#' @description this will input a data frame of ENSG counts and find the entrez and hgnc gene names and then collapse by gene name.  this is used for qusageTablesFromWGCNA to collapse the tmm normalized ensGID counts into tmm normalize gen name bundles.  arkas collapesBunders(kexp,gene_name) will give you counts(..) of gene_name bundles which is not exactly the same as using tmm normalized ENSGIDs and then collapsing tmm ENSGID counts by gene name.  slight difference but qusage requires it. we find that results are better if you first collapse by gene name then tmm normalize. this function is mostly useful for tpm.
